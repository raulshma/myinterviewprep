...
import { Check, Loader2, Sparkles, Zap, Shield, Infinity, Settings, X, HelpCircle, AlertCircle, ArrowUp, ArrowDown, Clock, CalendarClock } from 'lucide-react';
...
function PlanChangeDialog({ open, onOpenChange, type, fromPlan, toPlan, isLoading, onConfirm }: PlanChangeDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="sm:max-w-md p-0 overflow-hidden bg-card/95 backdrop-blur-xl border-white/10">
        <div className="flex flex-col items-center text-center p-8">
          ...
          {priceChange && (
            ...
            >
              <div className="flex-1 text-center">
                ...
                <p className="text-lg font-semibold text-muted-foreground line-through">${priceChange.from}/mo</p>
              </div>
              ...
              <div className="flex-1 text-center">
                ...
                <p className={`text-lg font-semibold ${priceChange.isIncrease ? 'text-primary' : 'text-amber-500'}`}>
                  ${priceChange.to}/mo
                </p>
              </div>
            </motion.div>
          ...
          {isUpgrade && (
            ...
            >
              ...
              <div className="space-y-2">
                {toPlan.features.slice(0, 4).filter(f => f.included).map((feature, i) => (
          ...
          {!isUpgrade && !isCancel && (
            ...
            >
              ...
              <div className="space-y-2">
                {PRICING_TIERS.find(t => t.id === fromPlan.toLowerCase())?.features
                  .filter(f => f.included && !toPlan.features.find(tf => tf.name === f.name)?.included)
                  .slice(0, 3)
                  .map((feature, i) => (
...
export function UpgradePageContent({ profile }: UpgradePageContentProps) {
  ...
  const openPlanChangeDialog = (tier: PricingTier, type: 'upgrade' | 'downgrade') => {
  ...
  const handleConfirmPlanChange = async () => {
    ...
    try {
      if (dialogState.type === 'upgrade') {
        ...
        const targetPlan = dialogState.targetTier.id === 'free' ? 'FREE' : 'PRO';
  ...
  const canUpgradeTo = (tierId: string): boolean => {
    ...
    if (profile.plan === 'FREE' && (tierId === 'pro' || tierId === 'max')) return true;
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  ...
                  <span className="font-medium text-foreground">
                    {new Date(profile.subscriptionCancelAt).toLocaleDateString('en-US', {
                      weekday: 'long',
                      month: 'long',
                      day: 'numeric',
                      year: 'numeric',
                ...
                <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                  ...
                  <p className="text-sm text-emerald-400">
                    You still have full access to all {profile.plan} features until then
                  </p>
                </div>
              </div>
            </div>
