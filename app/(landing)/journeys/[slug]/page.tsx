...
import { getPublicRoadmapBySlug } from '@/lib/actions/public-roadmaps';
import { getObjectiveTitle } from '@/lib/utils/lesson-utils';
import type { PublicRoadmap, PublicRoadmapNode } from '@/lib/db/schemas/visibility';
...
export default function PublicJourneyDetailPage() {
  ...
  useEffect(() => {
    async function loadRoadmap() {
      try {
        const data = await getPublicRoadmapBySlug(slug);
        if (!data) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        {/* Hero Section */}
        <section className="pt-12 pb-8 md:pt-20 md:pb-12 px-6 bg-gradient-to-b from-secondary/30 to-background">
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {roadmap.nodes.reduce((acc, n) => acc + n.learningObjectives.length, 0)} objectives
                  </span>
                </div>
              </div>
            </motion.div>
