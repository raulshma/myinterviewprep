...
import { getPublicRoadmaps } from '@/lib/actions/public-roadmaps';
...
function PublicJourneyCard({ roadmap, index }: { roadmap: PublicRoadmap; index: number }) {
  return (
    <motion.div
    ...
    >
      <Link href={`/journeys/${roadmap.slug}`}>
        <article className="group relative flex flex-col h-full bg-card hover:bg-gradient-to-br hover:from-card hover:to-accent/5 rounded-3xl border border-border/60 hover:border-primary/20 overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-1">
            ...
            {isLoading ? (
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {roadmaps.map((roadmap, index) => (
                  <PublicJourneyCard key={roadmap.slug} roadmap={roadmap} index={index} />
                ))}
              </div>
            )}
          </div>
