...
What if you didn't have to think about which related data to load upfront? **Lazy loading** automatically fetches related entities the moment you access them—like magic! But as Spider-Man's uncle said: "With great power comes great responsibility."
...
```
📦 Need milk? → Order it → Delivered!
📦 Need bread? → Order it → Delivered!
📦 Need cheese? → Order it → Delivered!
```
...
var posts = blog.Posts;  // 🪄 EF Core automatically fetches posts!
```
...
│                                                              │
│  var posts = blog.Posts;  // You access the property        │
│  // → EF Core intercepts this access                        │
│  // → SQL: SELECT * FROM Posts WHERE BlogId = @blogId       │
│  // → Returns the posts to you                              │
└─────────────────────────────────────────────────────────────┘
```
...
public class Blog
{
  ...
  steps={[
    ...
    },
    {
      ...
      explanation: "Without 'virtual', EF Core can't create the proxy—navigation stays null!"
    }
  ]}
/>
  ...
  steps={[
    {
      ...
      explanation: "Collection and reference navigations—both virtual"
    },
    {
  ...
  steps={[
    {
      ...
      explanation: "This is the '1' in N+1 — fetching the parent entities"
    },
    {
      lineNumbers: [9],
      highlight: "Hidden Queries",
      explanation: "Each iteration triggers a new query — the 'N' in N+1"
    },
    {
...
With Lazy Loading:
┌──────────┐     ┌────────────────────────────────────┐
│   App    │────▶│ Query 1: Get all blogs             │
└──────────┘     └────────────────────────────────────┘
      │          ┌────────────────────────────────────┐
      ├─────────▶│ Query 2: Get posts for blog 1      │
      │          └────────────────────────────────────┘
      │          ┌────────────────────────────────────┐
      ├─────────▶│ Query 3: Get posts for blog 2      │
      │          └────────────────────────────────────┘
      │          ... 97 more queries ...
      │          ┌────────────────────────────────────┐
      └─────────▶│ Query 101: Get posts for blog 100  │
                 └────────────────────────────────────┘

With Eager Loading:
┌──────────┐     ┌────────────────────────────────────┐
│   App    │────▶│ Query 1: Get all blogs WITH posts  │
└──────────┘     └────────────────────────────────────┘
                 (Just 1 query!)
```
...
// "Cannot access a disposed context instance"`}
  steps={[
    ...
    },
    {
      ...
      explanation: "Lazy loading needs the context alive—but it's gone!"
    }
  ]}
/>
