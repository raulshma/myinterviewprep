...
What if you want the on-demand flexibility of lazy loading but without the magic and hidden queries? **Explicit loading** gives you full control—you decide exactly when to load related data, and you can see it happening in your code.
...
```
📞 "Hello, I'd like to order the Posts for Blog #5"
🍽️ "Coming right up!"
✅ Posts are now loaded
```
...
The real power of explicit loading is the `Query()` method—it lets you filter and manipulate what gets loaded:
...
// Only the 5 most recent published posts!`}
  steps={[
    ...
    },
    {
      ...
      explanation: "Chain any LINQ methods—Where, OrderBy, Take, Skip, etc."
    }
  ]}
/>
...
<InfoBox type="tip">
**Memory efficiency!** Using Query().CountAsync() executes `SELECT COUNT(*)` in the database—no entities are loaded into memory.
</InfoBox>
...
Do you need related data?
         ...
         ▼
    ┌────────────┐
    │   Always?  │───Yes───▶ Use Eager Loading
    └────────────┘
         │ No
         ▼
    ┌────────────┐
    │ Sometimes? │───Yes───▶ Use Explicit Loading
    └────────────┘
         │ No
         ▼
  ...
  steps={[
    {
      ...
      explanation: "Load just the blog—no posts yet"
    },
    {
