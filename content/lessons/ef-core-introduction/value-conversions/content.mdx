...
Master **Value Conversions** – EF Core's powerful feature that transforms data between your C# types and database storage. Store enums as strings, encrypt data, serialize objects, and more!
...
// What does MountType = 2 mean? 🤷
```

With conversions, use expressive types:

```csharp
// 😊 With value conversions
public class Rider
{
    public int Id { get; set; }
...
// "Unicorn" is stored in DB as the string "Unicorn" ✨
```
...
| Converter Type | From → To | Use Case |
|:---------------|:----------|:---------|
| `EnumToStringConverter` | Enum → String | Readable database values |
| `EnumToNumberConverter` | Enum → Int | Compact storage |
| `BoolToStringConverter` | Bool → "Y"/"N" | Legacy database compatibility |
| `DateTimeToTicksConverter` | DateTime → Long | High-precision timestamps |
| `GuidToStringConverter` | Guid → String | Database compatibility |
...
modelBuilder.Entity<Order>()
    .Property(o => o.Price)
    .HasConversion(
        v => v.Amount,           // Dollars → decimal
        v => new Dollars(v));    // decimal → Dollars`}
  steps={[
    {
...
order.Price = new Euros(90);  // ❌ Compile error! Can't mix currencies
```
...
{
    // All DateTime properties → UTC conversion
    configurationBuilder
        .Properties<DateTime>()
        .HaveConversion<UtcDateTimeConverter>();
    
    // All Dollars properties → decimal conversion
    configurationBuilder
        .Properties<Dollars>()
        .HaveConversion<DollarsConverter>();
    
    // All enums → string storage
    configurationBuilder
        .Properties<Enum>()
...
modelBuilder.Entity<User>()
    ...
    .HasConversion<long>();  // DateTime → Ticks (long)
